...
from utils import extract_data_trafi, extract_links, timer, url_checker, get_last_date, dataset_path, csv_file_path, info_csv_path, txt_files_path, data_path, sender
...
def main():
    ...
    for EMAIL, PASSWORD in zip(EMAILS, PASSWORDS):
        ...
        last_date = get_last_date(mail_path)
        if last_date is None:
            status, data = mail.search(None, '(FROM {0})'.format(sender))
        else:
            t_date = last_date.split(" ")
            date = "-".join(t_date[1:4])
            status, data = mail.search(None, '(FROM {0})'.format(sender), '(SINCE {0})'.format(date))
        date_list = []
        links_list = []
        last_dates = []
        csv_paths = []
        nb_csv = len(os.listdir(dataset_path / mail_path / data_path))
        nb_url = 0
        count = 0
        for num in data[0].split()[count:]:
            ...
            date = msg.get("Date")
            if last_date is not None and mktime_tz(parsedate_tz(date)) <= mktime_tz(parsedate_tz(last_date)):
                continue
            links = []
